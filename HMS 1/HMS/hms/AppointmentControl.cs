...
namespace HMS
{
    public partial class AppointmentControl : UserControl
    {
        ...
        private void LoadAppointments(string search = "")
        {
            ...
            {
                string query = @"SELECT a.*, p.FirstName + ' ' + p.LastName as PatientName, d.FirstName + ' ' + d.LastName as DoctorName FROM tblAppointment a LEFT JOIN tblPatient p ON a.PatientID = p.PatientID LEFT JOIN tblDoctor d ON a.DoctorID = d.DoctorID WHERE a.IsDeleted = 0 AND (CAST(a.AppointmentID AS VARCHAR) LIKE @search OR p.FirstName + ' ' + p.LastName LIKE @search OR d.FirstName + ' ' + d.LastName LIKE @search OR a.Status LIKE @search OR a.Reason LIKE @search)";
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            if (form.ShowDialog() == DialogResult.OK)
            {
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    string query = @"INSERT INTO tblAppointment (PatientID, DoctorID, Date, Time, Reason, Status, IsDeleted) 
                               VALUES (@PatientID, @DoctorID, @Date, @Time, @Reason, @Status, 0)";
                    ...
                    cmd.Parameters.AddWithValue("@Date", form.dtpDate.Value.Date);
                    cmd.Parameters.AddWithValue("@Time", form.dtpTime.Value.TimeOfDay);
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            form.dtpDate.Value = Convert.ToDateTime(row.Cells["Date"].Value);
            form.dtpTime.Value = DateTime.Today.Add(TimeSpan.Parse(row.Cells["Time"].Value.ToString()));
            ...
            if (form.ShowDialog() == DialogResult.OK)
            {
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    string query = @"UPDATE tblAppointment SET PatientID=@PatientID, DoctorID=@DoctorID, Date=@Date, Time=@Time, Reason=@Reason, Status=@Status WHERE AppointmentID=@AppointmentID";
                    SqlCommand cmd = new SqlCommand(query, con);
                    cmd.Parameters.AddWithValue("@PatientID", form.cmbPatient.SelectedValue);
                    cmd.Parameters.AddWithValue("@DoctorID", form.cmbDoctor.SelectedValue);
                    cmd.Parameters.AddWithValue("@Date", form.dtpDate.Value.Date);
